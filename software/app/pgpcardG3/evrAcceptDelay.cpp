...
int main (int argc, char **argv) {
  ...
  while( ( c = getopt( argc, argv, "d:l:e:h" ) ) != EOF ) {
    switch(c) {
      ...
      case 'l':
        ...
        break;
      case 'h':
        printf("usage: %s -d device -l lane -e delay\n", argv[0]);
        return 0;
        break;
      default:
  ...
  unsigned long stuff = ((l << 28) | (e&0xfffffff));
  p->model = sizeof(p);
  p->cmd   = IOCTL_Evr_AcceptDelay;
  p->data  = (__u32*) stuff;
  write(fd, p, sizeof(*p));

  cout << endl;
  cout << "Wrote Evr accept delay " << e << " to lane " << l << endl << endl;
